...
from Network import Deeplab_v2, ResNet_segmentation
...
Training:
'poly' learning rate, different learning rates for different layers
"""
...
class ModelRunner(object):
        ...
        for step in range(self.conf.num_steps + 1):
            ...
            if step % self.conf.save_interval == 0:
                loss_value, images, labels, preds, summary, _ = self.sess.run(
                    [self.reduced_loss, self.image_batch, self.label_batch, self.pred,
                     self.total_summary, self.train_op], feed_dict=feed_dict)
                self.summary_writer.add_summary(summary, step)
                self.save(self.saver, step)
            else:
                loss_value, _ = self.sess.run([self.reduced_loss, self.train_op], feed_dict=feed_dict)
    ...
    def test(self):
        ...
        # load checkpoint
        self.load(self.loader, self.conf.modeldir + '/model.ckpt-' + str(self.conf.valid_step))
        ...
        for step in range(self.conf.valid_num_steps):
            preds, _, _, c_matrix = self.sess.run([self.pred, self.accu_update_op,
                                                   self.mIou_update_op, self.confusion_matrix])
    ...
    def predict(self):
        ...
        for step in range(self.conf.test_num_steps):
            ...
            im.save(self.conf.out_dir + '/prediction/{}_mask.png'.format(img_name))
    ...
    def train_setup(self):
        ...
        with tf.name_scope("create_inputs"):
            reader = ImageReader( self.conf.data_dir, self.conf.data_list, input_size, self.conf.random_scale,
                                  self.conf.random_mirror, self.conf.ignore_label, IMG_MEAN, self.coord)
        ...
        self.total_summary = tf.summary.image('images',tf.concat(axis=2, values=[images_summary, labels_summary,
                                                                                 preds_summary]),max_outputs=2)
    ...
    def test_setup(self):
        # Load reader
        with tf.name_scope("create_inputs"):
            reader = ImageReader( self.conf.data_dir, self.conf.valid_data_list, None, False, False,
        ...
        # Pixel accuracy
        self.accu, self.accu_update_op = tcm.streaming_accuracy(self.pred, gt, weights=weights)

        # mIoU
        self.mIoU, self.mIou_update_op = tcm.streaming_mean_iou(self.pred, gt, self.conf.num_classes, weights)

        # confusion matrix
        self.confusion_matrix = tcm.confusion_matrix(self.pred, gt, num_classes=self.conf.num_classes, weights=weights)
    ...
    def predict_setup(self):
        # Load reader
        with tf.name_scope("create_inputs"):
            reader = ImageReader(self.conf.data_dir, self.conf.test_data_list, None, False, False,
                                 self.conf.ignore_label, IMG_MEAN, self.coord)
            image, label = reader.image, reader.label  # [h, w, 3 or 1]
        # Add one batch dimension [1, h, w, 3 or 1]
    ...
    def save(self, saver, step):
        checkpoint_path = os.path.join(self.conf.modeldir, 'model.ckpt')
