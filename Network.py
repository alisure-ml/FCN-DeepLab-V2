...
This script supports several settings (you need to specify in main.py):
...
class Deeplab_v2(object):
    ...
    def _ASPP(self, x, num_o, dilations):
        ...
        for i, d in enumerate(dilations):
            o.append(self._dilated_conv2d(x, 3, num_o, d, name='fc1_voc12_c{}'.format(i), biased=True))
...
class ResNet_segmentation(object):
    ...
    def __init__(self, inputs, num_classes, phase, encoder_name):
        if encoder_name not in ['res101', 'res50']:
            ...
            raise Exception('encoder_name ERROR!')
    ...
    def _bottleneck_resblock(self, x, num_o, name, half_size=False, identity_connection=True):
        ...
        if not identity_connection:
            ...
            o_b1 = self._batch_norm(o_b1, '%s/bottleneck_v1/shortcut' % name, False, activation_fn=None)
        else:
            o_b1 = x
        # branch2
        o_b2a = self._conv2d(x, 1, num_o / 4, first_s, name='%s/bottleneck_v1/conv1' % name)
        o_b2a = self._batch_norm(o_b2a, '%s/bottleneck_v1/conv1' % name, is_training=False, activation_fn=tf.nn.relu)

        o_b2b = self._conv2d(o_b2a, 3, num_o / 4, 1, name='%s/bottleneck_v1/conv2' % name)
        o_b2b = self._batch_norm(o_b2b, '%s/bottleneck_v1/conv2' % name, is_training=False, activation_fn=tf.nn.relu)
    ...
    def _dilated_bottle_resblock(self, x, num_o, dilation_factor, name, identity_connection=True):
        ...
        if not identity_connection:
            ...
            o_b1 = self._batch_norm(o_b1, '%s/bottleneck_v1/shortcut' % name, is_training=False, activation_fn=None)
        else:
            o_b1 = x
        # branch2
